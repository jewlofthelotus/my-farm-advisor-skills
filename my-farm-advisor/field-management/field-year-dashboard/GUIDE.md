...
description: Generate a single multi-panel dashboard image for one field and growing season. Four vertically stacked panels (NDVI, precipitation, temperature, cumulative GDD) share a common DOY axis with event annotations and a crop-strategy-informed caption.
license: MIT
...
```
  Field {id} — {year} Growing Season
  ┌──────────────────────────────────────┐
  │ NDVI        [bars + polynomial trend]│ NDVI events annotated
  ├──────────────────────────────────────┤
  │ Precipitation [bars + cumulative]    │ Heavy rain / dry spell labels
  ├──────────────────────────────────────┤
  │ Temperature  [bars only]             │ Heat stress, frost, cool periods
  ├──────────────────────────────────────┤
  │ Cumulative GDD [bars + cumulative]   │ Growth stage markers
  ├──────────────────────────────────────┤
  │ Caption box: crop, thresholds, events│
  └──────────────────────────────────────┘
  Day of Year (1–365)
```
