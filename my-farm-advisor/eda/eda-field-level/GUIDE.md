...
|---|---|---|
| `eda_geospatial_map.py` | `cross_grower_field_centroid_map.png`, `cross_grower_field_centroids.csv` | Geospatial map of field centroids with area-scaled markers on state-outline basemap; CSV of centroid coordinates with area_acres |
| `eda_field_boundaries.py` | `cross_grower_field_area_cumulative_stacked_bar.png`, `cross_grower_field_area_histogram.png`, `cross_grower_field_count_vs_acreage.png`, `cross_grower_field_boundary_metrics.csv`, `cross_grower_field_count_acreage_summary.csv` | Cumulative field area bar chart, faceted field-size histograms, field count vs total acreage paired bars; per-field geometry CSV and per-grower summary CSV |
| `eda_field_cdl.py` | `cross_grower_cdl_crop_composition_flow.png`, `{grower}_cdl_crop_rotation_heatmap.png` (per grower), `{grower}_cdl_crop_composition_by_year.png` (per grower), `cross_grower_cdl_composition.csv`, `cross_grower_cdl_crop_totals_by_year.csv` | Cross-grower crop composition flow chart, per-grower rotation heatmaps, per-year composition bars; full CDL composition CSV and aggregated totals CSV |
| `eda_field_weather.py` | `cross_grower_weather_average_cumulative_annual_precip.png`, `{grower}_weather_mean_daily_precip_by_season.png` (per grower), `cross_grower_weather_temp_precip_dual_axis.png`, `cross_grower_weather_annual_precipitation.csv`, `cross_grower_weather_annual_summary.csv` | Cumulative annual precip bar chart, per-grower daily precip by season, precip + temp dual-axis; annual precip CSV and annual summary CSV |

### Report generation script

| Script | Outputs | What it produces |
|---|---|---|
| `generate_reports.py` | `eda_field_level_report.html`, `eda_field_level_report.docx` | Aligned HTML and DOCX reports combining all analysis outputs with category stories, plot descriptions, and CSV references |
